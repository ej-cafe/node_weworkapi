...
describe("WXBizMsgCrypt TestSuite", () => {
    ...
    describe('decryptMessage TestCase', () => {
        it('Test decryptMessage', async () => {
            ...
            <AgentID><![CDATA[218]]></AgentID>
                ...
                nonce = "1372623149";
            let wxBizMsgCrypt = WXBizMsgCrypt(token, encodingAesKey, corpId);
            let message = await wxBizMsgCrypt.decryptMessage(signature, timestamps, nonce, postData);
    ...
    describe('encryptMessage TestCase', () => {
        it('Test encryptMessage', async () => {
            ...
            let replyMsg = '<xml><ToUserName>ww1436e0e65a779aee</ToUserName><FromUserName>ChenJiaShun</FromUserName><CreateTime>1476422779</CreateTime><MsgType>text</MsgType><Content>你好</Content><MsgId>1456453720</MsgId><AgentID>1000002</AgentID></xml>';
            let wxBizMsgCrypt = WXBizMsgCrypt(token, encodingAesKey, corpId);
            let message = await wxBizMsgCrypt.encryptMessage(replyMsg, timestamp, nonce);
            expect(message).toMatchSnapshot();
        });
    });
});